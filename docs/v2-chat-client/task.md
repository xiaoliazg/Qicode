...
| 新建 | `src/qicode/config.py` | `Config` / `ProviderConfig`、`load`、校验 |
| 新建 | `src/qicode/prompt.py` | `SYSTEM_PROMPT`、`MASCOT_BANNER`、`render_banner` |
...
| 新建 | `src/qicode/llm/openai_provider.py` | openai 适配器 |
| 新建 | `src/qicode/tui/__init__.py` | TUI 包标识 |
| 新建 | `src/qicode/tui/app.py` | `QicodeApp`、状态机、`run` |
| 新建 | `src/qicode/tui/stream.py` | `_consume_stream`、`_tick` 计时 |
| 新建 | `src/qicode/tui/select.py` | provider 选择（`OptionList`） |
| 新建 | `src/qicode/tui/view.py` | 渲染拼装、状态栏、错误样式、markdown 定型 |
| 新建 | `src/qicode/cli.py` | 入口装配 |
...
2. 定义 `MASCOT_BANNER`：吉祥物图案。用**带背景色的空格**逐格拼像素画——空格宽度恒为 1 列，任何终端、任何字体都不会错位（实心块字符 `█` 的显示宽度在部分终端会在 1/2 格之间摇摆）。橙色头发 + 猫耳 + 腮红，约 12 列 × 6 行。
3. 实现 `def render_banner(version: str, cwd: str) -> str`：拼出"吉祥物 + Qicode vX + cwd + 就绪提示行"。

**验证：** `python -c "from qicode.prompt import render_banner; print(render_banner('0.1.0', '/tmp'))"` 输出含三要素与提示行。
...
2. 定义 `class QicodeApp(App)`：构造参数 `providers: list[ProviderConfig]`；初始化 `state`、`provider: Provider | None`、`conv = Conversation()`、`cur_reply = ""`、`turn_start = 0.0`、`_stream_task = None`、`_timer = None`。
...
6. `def main()`（在 `cli.py` 中调用）：`QicodeApp(providers).run()`。
...
**依赖：** T9、T5
**步骤：**
1. 在 `app.py` 给 `QicodeApp` 添加 `async def submit(self, text: str)`：
...
3. 状态栏：用 Rich 的 `Text`/`Table.grid` 左 `provider.name`、右 `provider.model`，两端对齐；
   写到 `#statusbar: Static`。
4. 完成块（追加到 `RichLog`）：
    - `user_block(text)` = `Text("● " + text, style="bold")` 或纯文本（无 You/Qicode 文字标签）；
...
**步骤：**
1. `def main() -> None`：
    - `try: cfg = config.load(".qicode/config.yaml")`；`except ConfigError as e: print(e, file=sys.stderr); sys.exit(1)`。
    - 可选：先 `print(prompt.render_banner(__version__, os.getcwd()))`，或交给 TUI 在 `on_mount` 写 `RichLog`（二选一保持一致；本项目用后者）。
    - `QicodeApp(cfg.providers).run()`；若抛非 KeyboardInterrupt 异常，`print(...)` 并 `sys.exit(1)`。
