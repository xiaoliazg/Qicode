...
> ---
>
> **六次回填：2026-09-23（T23，输入框光标）。** 居居又看真界面，这次指着输入框：
> **光标一到中间就吞掉一个字**。
>
> 根因是 T21 那段「在空白格画一根细竖线 `▏`」的**实现**里有两个错叠在一起——把
> 「字符索引」当「显示列」用，又拿 `strip.crop` 判断「这格空不空」（而它切不出完整的
> 宽字符时会补空格，于是**中文的每一格都判成空**）。**中文输入必然中招**；T21 当时的
> 验证用例是 `hello`，一个字符一格，两个错恰好都显不出形。
>
> 修法换了个方向：**把光标还给终端自己画**（Textual 启动时会把它藏掉）。见文末
> 「T23 修复记录」。
...
> **T23 补记（2026-09-23）：这句「能力上限」说早了。** 上限是有的——**在格子里**确实
> 画不出不占格的竖线——但结论不该停在「所以分两路」。还有第三条路：**别在格子里画，
> 用终端自己的硬件光标**（Textual 启动时会把它藏掉，显式写一句 `\x1b[?25h` 就能要回来）。
> 它就是画在格子边界上的，两路都省了。这一节下面那两路里，`render_line` 那条还带着
> 两个 bug（详见文末「T23 修复记录」）。
...
## T23 输入框光标交给终端自己画（2026-09-23）

### 起因

居居接着看真界面，这次指着输入框：**光标一到中间就吞掉一个字**。截图两张——
`> 你 | 干_`（光标右边那个字整个没了）、`正常的一个竖线不|就好啦，`（同）。补一句：

> 就是正常的一个竖线不就好啦，

### 根因

T21 那段「在光标那格里画一根细竖线 `▏`」的 `render_line`，**两个错叠在一起**：

1. `visible_col = col - self.scroll_offset.x` —— 拿到的 `col` 是**字符索引**
   （`cursor_location.x` 的语义），却被当成**显示列**用。一个中文两列宽，两者不等。
2. `cell = strip.crop(visible_col, visible_col + 1)` 然后 `if cell.text.strip():`
   —— 意图是「这格有字就别覆盖」。但 Rich 的 `Strip.crop` 在**切不出一个完整字符**
   时会拿空格补齐，于是**中文的每一格都裁出 `' '`**，「有空位」永远成立，
   `▏` 就无条件盖上去，把那格的字吃掉。

两个错必须**同时**在场才看得见：只错 ①，光标位置偏但字还在；只错 ②，ASCII 下
`crop` 切得准，判断是对的。**中文输入必然两个都踩**。探针实测（`/tmp/probe_cursor.py`）：
光标在第 13 / 10 / 9 / 0 个字符处，那行分别成了
`'就是正常的一 ▏竖线不就好啦'` / `'就是正常的▏ 个竖线不就好啦'` /
`'就是正常 ▏一个竖线不就好啦'` / `'▏ 是正常的一个竖线不就好啦'`——**每次都吃一个**。

### 为什么 T21 没发现

T21 的验证表（本文档上面那节）用的是 `❯ hello▏`、`hel` + 下划线 + `l`——
**全是 ASCII**。一个字符一格，`col` 恰好等于显示列，`crop` 恰好裁得出完整字符，
两个错都不显形。这不是虚勾：当时确实照着 checklist 验了，**是验收用例的取样面
只盖住了单字节**，跟 T22 那条「从没带着工具跑过 ollama」是同一类**覆盖缺口**。

### 修法：不画了，把光标还给终端

方向换掉，不是在 `render_line` 里修那两个公式。理由是一条**能力上限**：

> 终端是「一格一个字符」的网格，**在格子里画不出不占格的竖线**。想在两个字中间
> 插一条线，就得占掉一格——要么把那格的字挤掉（就是现在这个吞字），要么让整行
> 往右挪（光标右边的字每敲一下都在抖）。

而**终端自己的硬件光标画在格子的边界上**，不占格子，压在字上也不丢字——它天生
就是干这个的。T21 绕开它是因为 Textual 启动时把它藏了（
`textual/drivers/linux_driver.py` 的 `start_application_mode()` 里一句 `\x1b[?25l`），
当时没往下追这一步，就自己在格子里画了一根。实测（tmux 80×12）：不干预时
`#{cursor_flag}` 是 `0`；显式写一次 `\x1b[?25h` 之后变 `1`，并且跟着输入走
（`#{cursor_x}` 2 → 7）。所以路是通的。

具体三处（`src/qicode/tui/view.py`、`src/qicode/tui/app.py`）：

- **删掉整个 `render_line`**，连同 `CURSOR_BAR = "▏"`。那两个公式和那根线一起消失。
- `PromptArea` 挂 `on_focus` / `on_blur`，写 `\x1b[?25h` / `\x1b[?25l`。
  **位置不用我们管**：Textual 每次渲染都会 `Control.move_to(App.cursor_position)`，
  而 `TextArea` 在聚焦、光标移动、选区变化时都会更新这个值，我们只管开关。
- CSS 把 `TextArea .text-area--cursor` 的橙色下划线清成中性
  （`color` / `background` 都给 `ansi_default`，`text-style: none`），
  屏幕上就只剩终端那一根，不会冒出两个光标。
- `cursor_blink = False` **留着**，但理由换了：以前是「别闪」，现在是「别白刷」——
  那一格的样式已经清成中性，`_draw_cursor` 每 0.5 秒把它标脏重绘一次，画出来跟
  前一帧一模一样。

### 三个踩过的坑

1. **CSS 规则不能写空。** `textual/_text_area_theme.py` 的 `apply_css` 逻辑是
   `cursor_style = get_style("text-area--cursor"); if cursor_style: ... else: <退回内置默认>`
   ——**拿到的样式为空就退回默认的反色方块**。一条空规则等于没写，T21 好不容易
   去掉的方块会原样回来。所以三个属性一个都不能省。
2. **失焦得我们自己收。** `TextArea._watch_has_focus` 在失焦时**不会**重置
   `App.cursor_position`，只 `_pause_blink(visible=False)`。不写 `on_blur`，
   焦点跑去选择列表了光标还杵在输入框里，看着像那儿还能打字。
3. **为什么挂在焦点事件上而不是 `on_mount` 开一次。** 光标是「键盘现在打给谁」的
   指示物，得跟着焦点走（选择列表、被点中的对话区都能抢走焦点）。时序上也安全：
   `App._process_messages` 先建 driver、先跑 `start_application_mode()`，然后才进
   消息循环跑 mount——`on_focus` 触发时 driver 一定就绪。

### 反向证明（变异测试）

把 T21 那个有 bug 的 `render_line` 原样装回去，确认用例会红：

| 变异 | 结果 |
|---|---|
| 装回 T21 的 `render_line` + `CURSOR_BAR` | 红 1 条（`test_cjk_input_is_never_eaten_by_the_cursor`） |

带出来的断言原文，正好把两个错一起钉住：

```
AssertionError: 光标停在第 0 个字符处时，那行显示成了 '│ ❯ ▏ 是正常的一个竖线 ok 不就好啦 │'
```

还原后 `diff` 逐字节确认无残留，`grep -c "MUTANT\|render_line"` 返回 `0`。

### 验证

| 项 | 方式 | 结果 |
|---|---|---|
| 四个门禁 | ruff check / ruff format / mypy / pytest | 全绿，**282 passed**（改前 279，+3） |
| 光标真的放出来了 | tmux 100×30 实跑，`display -p '#{cursor_flag}'` | 启动即 `1`（改前是 `0`） |
| 位置准不准（中文） | 输入 `就是正常的一个竖线不就好啦`（13 字） | `x=30`——4 + 13×2，**双宽算对了** |
| 位置跟着方向键走 | `Left` ×3 → `x=24`；再 ×10 → `x=4`；`Right` ×6 → `x=16` | 每一步都对 |
| 字有没有被吃 | 上面每个位置都 `capture-pane -p` 抓整行 | 13 个字**全在**，一个不少 |
| ASCII 不回归 | 输入 `hello world` | 正常 |
| 失焦要收回 | 多 provider 配置，启动选择列表 | `cursor_flag=0`，选中 Enter 之后回到 `1` |
| 跟 T22 不打架 | 完整对话一轮（ollama 降级路径） | `● 用一句话介绍你自己` / `● 我是 Qicode，一个运行在用户终端里的 AI 助手。 5.5s`，T22 的降级提示照常出现 |
| 对话后再验一次 | 回车后输入 `中间不许吞字` | 完好；`x=16` → `Left` ×4 → `x=8` |
| 退出 | `Ctrl+C` | 回放正常，无残留 |

### 顺带还原的一处

T21 那节记的「顺带修掉的一处小瑕疵」——占位符为了给 `▏` 让位改成了
`" Send a message..."`（开头一个空格）——**这次还原成 `"Send a message..."`**。
现在光标不占格子了，占位符不用再让位，那个空格也就没有存在的理由。

### 已知不覆盖

- **tmux 里看不到光标的形状**（`capture-pane` 抓的是网格内容，硬件光标不在里面），
  所以「它长什么样」是居居在真终端里用眼睛确认的（截图 `OK|哦`，细竖线在正中间，
  两个字符都在）。自动化这边只能验到「开关写了、位置对了」这一层。
- **没验过不支持 `CSI ? 25 h` 的终端**。这是 DEC 私有模式，从 VT220 起就是事实标准，
  真遇到不吃它的终端，光标会一直藏着（退回「看不见光标」而不是「吞字」——
  更保守的那个失败方向）。
- **没用例覆盖「焦点在对话区时打字」**：`on_blur` 收光标这件事有用例，但焦点离开
  输入框之后键盘事件到底由谁接，本阶段没管（对话区目前不吃键）。
