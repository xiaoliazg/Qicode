...
#: 有工具时的说明（v2 起的正文）。
_TOOLS_AVAILABLE = """\
你可以使用工具：读写和修改文件、执行 shell 命令、按模式查找文件、搜索代码内容。
需要事实、需要动手的时候，**先调用相应的工具**，拿到结果再给简洁的答复；
不要凭记忆猜测文件里写了什么，也不要空口描述一个你没跑过的命令的输出。
工具返回的错误是给你的线索，照着它调整参数重试即可。"""

#: 没工具时的说明。
#:
#: **这段不是「有工具」那段的反话，是必须另写的一段。** 少了它、又确实没有工具时，
#: 模型会照着上一段的指示去「调用工具」——它在协议上其实调不了，于是就**在正文里
#: 演一遍**：写一句 `read_file({"path": "a.py"})` 然后接着编下面的内容。用户看到的
#: 是一段像模像样的假输出，比直接说「我读不了文件」危险得多。
#:
#: 所以这里做两件事：说清**能做什么**（缺什么就张口要，用户会把它贴进来），
#: 再点名**不许做什么**（那个形态的假动作）。第二句要具体到样子，不能只说
#: 「不要假装调用工具」——模型不知道「假装」在界面上长什么样。
_TOOLS_UNAVAILABLE = """\
本次会话**没有**可用的工具：你无法读写文件、无法执行命令、也无法搜索代码内容。
需要这些信息时，直接说明你需要哪份文件、或者需要用户执行哪条命令，让他把结果给你。
**不要**在回复里假装调用工具——比如写一段形如 `read_file({"path": "a.py"})` 的文字
再接着往下编内容。你并没有真的调用，用户看不到任何结果，那样写出来的东西是假的。"""


def system_prompt(provider_name: str, model: str, *, tools: bool = True) -> str:
    ...
    **为什么要把 provider / model 写进去。** 这两个值只有我们这边知道——它们是
    ...
    v2 加了工具，这里补一段「你是个能动手的 Agent」。**不把六个工具的参数说明抄
    进来**：那些由工具定义本身（`ToolDefinition`）随每次请求一起送给模型，是
    结构化的、永远跟实现同步的；在这儿再用自然语言重复一遍，只会白占上下文，
    还多一处「文档跟实现对不上」的地方。

    **`tools` 这个开关说的是「这次请求里到底带没带工具定义」**，不是「你想不想给」。
    调用方必须如实传：说了可以用工具却没给（或者反过来），模型的行为就跟说明对不上，
    而它对不上的方式多半是**编**。目前有两个调用点会传 `False`：

    - Anthropic 开了 `thinking: true`（发请求前就知道工具不可用）；
    - OpenAI 兼容侧撞上「这个模型不支持工具」的 400（撞了才知道，
      见 `qicode/llm/openai_provider.py` 的 `supports_tools`）。

    默认 `True` 是为了让「工具一直可用」那条路径不必显式传参。
    """
    return f"""\
...
{_TOOLS_AVAILABLE if tools else _TOOLS_UNAVAILABLE}
